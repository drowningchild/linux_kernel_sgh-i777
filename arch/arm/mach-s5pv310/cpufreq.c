...
#define ARMCLOCK_1704MHZ		1704000
#define ARMCLOCK_1600MHZ		1600000
#define ARMCLOCK_1504MHZ		1504000
...
enum cpufreq_level_index{
	L0, L1, L2, L3, L4, L5, L6, L7, L8, CPUFREQ_LEVEL_END,
};
#else
enum cpufreq_level_index{
...
void s5pv310_set_frequency(unsigned int old_index, unsigned int new_index)
{
	...
	if (freqs.old < freqs.new) {
		...
		if (is_curfreq_table &&
			(((freqs.old == 500000) && (freqs.new == 1000000)) ||
			((freqs.old == 200000) && (freqs.new == 800000))))
				change_s_value = 1;
		...
		if (is_curfreq_table &&
			(((freqs.old == 1000000) && (freqs.new == 500000)) ||
			((freqs.old == 800000) && (freqs.new == 200000))))
				change_s_value = 1;
		...
		unsigned int relation)
{
	...
	if (s5pv310_max_armclk == ARMCLOCK_1704MHZ) {
#ifdef CONFIG_FREQ_STEP_UP_L2_L0
		/* change L4 (L2) -> L0 */
		if ((index == L0) && (old_index > L6)) {
			index = L4;
		}
#else
			...
			enum cpufreq_level_request cpufreq_level)
{
	...
	if (s5pv310_max_armclk != ARMCLOCK_1704MHZ) {
		if (cpufreq_level != CPU_L0) {
			...
			printk(KERN_WARNING
				"[CPUFREQ]cpufreq locked to 1GHz in place of 1.704GHz\n");
		}
	}
		...
		unsigned long event, void *ptr)
{
	...
	case PM_SUSPEND_PREPARE:
		max = policy->max;
		min = policy->min;
		policy->max = policy->min = s5pv310_freq_table[L6].frequency;
		ret = cpufreq_driver_target(policy,
		s5pv310_freq_table[L6].frequency, DISABLE_FURTHER_CPUFREQ);
		if (WARN_ON(ret < 0))
			return NOTIFY_BAD;
#ifdef CONFIG_S5PV310_BUSFREQ
	...
	case PM_POST_SUSPEND:
		...
		ret = cpufreq_driver_target(policy,
		s5pv310_freq_table[L6].frequency, ENABLE_FURTHER_CPUFREQ);
		policy->max = max;
		policy->min = min;
#ifdef CONFIG_S5PV310_BUSFREQ
...
static int s5pv310_update_dvfs_table()
{
	...
	case 1704000:
		printk(KERN_INFO "armclk set max 1704MHz as default@@@@@\n");
		break;
